  ...
  head: () => ({
    meta: [
      { title: "Zarmina — Grace woven in every thread" },
      {
        name: "description",
        content:
          "A curated house of handcrafted South Asian luxury. Kashmiri phirans, chikankari, and heirloom formals for the modern wardrobe.",
      },
      { property: "og:title", content: "Zarmina — Grace woven in every thread" },
      {
        property: "og:description",
        content: "Curated handcrafted South Asian luxury. Timeless artistry for today.",
      },
...
const collections = [
  ...
  },
  {
    name: "Chikankari",
    tag: "Lucknow",
    image: artisanBrown.url,
    copy: "Whisper-fine white-on-white shadow work, stitched over months.",
  },
  {
    name: "Pakistani Formals",
    tag: "Occasion",
    image: productBlue.url,
    copy: "Ceremonial ensembles woven with heirloom zardozi and dabka.",
  },
  {
    name: "Lawn Collection",
    tag: "Seasonal",
    image: detailTeal.url,
    copy: "Featherlight cottons printed and embroidered for warmer days.",
  },
];

const products = [
  { name: "Firdaus Phiran", meta: "Aari on wool · Kashmir", price: "₹ 48,000", image: heroImg.url },
  { name: "Meher Kurta", meta: "Chikankari on mul · Lucknow", price: "₹ 22,500", image: artisanBrown.url },
  { name: "Noor Ensemble", meta: "Zardozi on raw silk", price: "₹ 72,000", image: productBlue.url },
  { name: "Zeba Abaya", meta: "Kashida on charcoal wool", price: "₹ 38,000", image: productBlack.url },
];
  ...
  return (
    <section className="relative h-dvh min-h-[720px] w-full overflow-hidden">
      <div className="absolute inset-0 ken-burns">
        <img src={heroImg.url} alt="Woman in emerald Kashmiri phiran" className="h-full w-full object-cover" />
      </div>
      ...
      <div className="relative z-10 flex h-full flex-col justify-end pb-24 md:pb-32 px-6 md:px-16">
        <div className="max-w-3xl">
          ...
          >
            Curated handcrafted fashion celebrating timeless artistry, exceptional
          ...
          >
            <Link to="/collections" className="btn-luxe-invert">Explore Collection</Link>
            <Link to="/about" className="btn-luxe-invert">Discover Our Story</Link>
          </div>
        </div>
      </div>
    ...
    <section className="py-28 md:py-40 px-6 md:px-16">
      <div className="mx-auto max-w-[1400px] grid gap-16 md:gap-24 md:grid-cols-[1fr_1.1fr] items-center">
        <Reveal className="relative aspect-[4/5] overflow-hidden bg-beige">
          <img src={detailTeal.url} alt="Embroidery detail" className="h-full w-full object-cover" />
        </Reveal>
        <div>
          <Reveal>
            <div className="eyebrow mb-8">
              <span className="hairline w-8 mr-3 align-middle" /> A quiet philosophy
            </div>
          </Reveal>
          <Reveal delay={120}>
          ...
          <Reveal delay={240}>
            <p className="mt-8 text-charcoal/75 text-lg leading-[1.8] max-w-xl font-serif">
              Zarmina is not a boutique. It is a considered archive of the artisans,
              regions, and traditions that shape South Asian dress. Each garment we
              curate is chosen for the years it has taken to learn — and the years it
              will live in your wardrobe.
            </p>
          </Reveal>
          <Reveal delay={340}>
            <Link to="/about" className="mt-10 inline-block eyebrow text-charcoal link-underline">
              Read the manifesto →
            </Link>
          </Reveal>
        </div>
    ...
    <section className="py-24 md:py-32 bg-ivory">
      <div className="px-6 md:px-16 mb-16 md:mb-24">
        <div className="mx-auto max-w-[1400px] flex flex-col md:flex-row md:items-end md:justify-between gap-6">
          ...
          <Reveal delay={200}>
            <Link to="/collections" className="eyebrow link-underline text-charcoal">
      ...
      <div className="grid gap-2 md:gap-3 md:grid-cols-2">
        {collections.map((c, i) => (
          <Reveal key={c.name} delay={i * 80}>
            <Link to="/collections" className="group relative block overflow-hidden aspect-[4/5] md:aspect-[5/6]">
              <img
                src={c.image}
                alt={c.name}
              ...
              <div className="absolute inset-0 flex flex-col justify-end p-8 md:p-14">
                ...
                <h3 className="font-display text-4xl md:text-5xl text-ivory">{c.name}</h3>
...
function Philosophy() {
  const points = [
    { n: "01", t: "Quality over quantity", d: "A single well-made garment outlives a season of trends." },
    { n: "02", t: "Timeless silhouettes", d: "Shapes that honour the body without asking anything of it." },
    { n: "03", t: "Thoughtful craftsmanship", d: "Every stitch is placed with intention by named artisans." },
    { n: "04", t: "Conscious curation", d: "We ship less, and choose slower — always." },
  ];
  return (
    <section className="py-28 md:py-40 bg-beige/60 px-6 md:px-16">
      <div className="mx-auto max-w-[1400px]">
        <Reveal><div className="eyebrow mb-8">Our philosophy</div></Reveal>
        <Reveal delay={100}>
          <h2 className="font-display text-5xl md:text-7xl text-charcoal max-w-3xl leading-[1.05]">
            Designed to last <em className="text-emerald">beyond seasons.</em>
          </h2>
        </Reveal>
          ...
          {points.map((p, i) => (
            <Reveal key={p.n} delay={i * 100}>
              <div className="border-t border-charcoal/25 pt-6">
                <div className="font-display text-gold text-3xl mb-4">{p.n}</div>
                <h3 className="font-display text-2xl mb-3 text-charcoal">{p.t}</h3>
                <p className="text-charcoal/70 leading-relaxed text-sm">{p.d}</p>
              </div>
            </Reveal>
          ))}
        </div>
    ...
    <section className="relative">
      <div className="grid md:grid-cols-2">
        <div className="relative min-h-[500px] md:min-h-[820px] overflow-hidden">
          <img src={artisanBrown.url} alt="Artisan hands" className="absolute inset-0 h-full w-full object-cover" />
        </div>
          ...
          <div className="max-w-lg">
            <Reveal>
              <div className="eyebrow text-gold mb-8">
                <span className="hairline w-8 mr-3 align-middle bg-gold" /> Hands that remember
              </div>
            </Reveal>
            <Reveal delay={100}>
            ...
            <Reveal delay={200}>
              <p className="mt-8 text-ivory/75 leading-[1.9] font-serif text-lg">
                Behind every embroidered motif is a family — a mother, a grandfather,
                an entire village whose fingers have memorised the same knots since
                childhood. We work directly with these ateliers so their names, their
                pace, and their fair earnings travel with every piece.
              </p>
            </Reveal>
            <Reveal delay={300}>
              <Link to="/journal" className="mt-10 inline-block eyebrow text-gold link-underline">
                Meet the artisans →
              </Link>
            </Reveal>
          </div>
    ...
    <section className="py-28 md:py-40 px-6 md:px-16">
      <div className="mx-auto max-w-[1500px]">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between mb-16 gap-6">
          <div>
            <Reveal><div className="eyebrow mb-6">The Atelier</div></Reveal>
            <Reveal delay={100}>
              ...
              <div className="group cursor-pointer">
                <div className="relative overflow-hidden aspect-[4/5] bg-beige">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4 opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500">
                    <div className="btn-luxe-invert w-full text-[0.65rem]">Quick view</div>
                  </div>
                </div>
                <div className="pt-5">
                  <h3 className="font-display text-2xl text-charcoal">{p.name}</h3>
                  <div className="mt-1 text-xs text-muted-foreground tracking-wide">{p.meta}</div>
  ...
  return (
    <section className="relative py-28 md:py-40 overflow-hidden">
      <div className="absolute inset-0">
        <img src={detailSleeve.url} alt="" className="h-full w-full object-cover" />
      ...
      <div className="relative z-10 mx-auto max-w-4xl px-6 md:px-10 text-center text-ivory">
        <Reveal>
          <img src={mark.url} alt="" className="h-16 mx-auto mb-8 opacity-90 invert" />
        </Reveal>
        <Reveal delay={100}>
...
function Testimonials() {
  ...
  return (
    <section className="py-28 md:py-40 bg-ivory px-6 md:px-16">
      <div className="mx-auto max-w-[1400px]">
        <Reveal><div className="eyebrow text-center mb-8">In their words</div></Reveal>
        <div className="grid gap-14 md:grid-cols-3 mt-14">
          {items.map((t, i) => (
            <Reveal key={t.a} delay={i * 120}>
              <figure className="text-center px-4">
                <div className="text-gold font-display text-5xl leading-none mb-6">"</div>
                ...
                <figcaption className="mt-8 eyebrow text-charcoal">
                  {t.a} <span className="text-muted-foreground">— {t.c}</span>
                </figcaption>
              </figure>
            </Reveal>
          ))}
...
function InstagramGallery() {
  const imgs = [heroImg.url, detailTeal.url, artisanBrown.url, productBlue.url, productBlack.url, detailSleeve.url];
  return (
    <section className="py-24 md:py-32 px-6 md:px-10">
      <div className="text-center mb-14">
        <Reveal><div className="eyebrow mb-4">@zarmina</div></Reveal>
        <Reveal delay={100}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-2">
        {imgs.map((src, i) => (
          <Reveal key={i} delay={i * 60}>
            <a href="#" className="group block relative aspect-square overflow-hidden bg-beige">
              <img
                src={src}
                alt=""
